...
from app.deps import get_db, get_current_user, redirect_to
...
async def dashboard(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    return templates.TemplateResponse(request, "dashboard.html", {"user": user, "ha_ok": ha_ok, "greeting": greeting})
...
async def fitness(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    if not user:
        return RedirectResponse(url=redirect_to(request, "ui/login"), status_code=302)
    return templates.TemplateResponse(request, "fitness.html", {"user": user})
