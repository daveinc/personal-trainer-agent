...
async def _create_ext_db_if_missing():
    host = os.getenv("DB_HOST", "").strip()
    port = int(os.getenv("DB_PORT", "3306") or "3306")
    name = (os.getenv("DB_NAME", "coach") or "coach").strip()
    user = os.getenv("DB_USER", "").strip()
    password = os.getenv("DB_PASSWORD", "").strip()
    try:
        import aiomysql
        ...
        logger.error(f"Could not create external DB '{name}': {e}")
...
async def init_db():
    ...
    if ext_engine:
        await _create_ext_db_if_missing()
        try:
